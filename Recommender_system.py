...
parser = argparse.ArgumentParser(
    description='Recommender syste mfor users or courses')
parser.add_argument('-c', '--course_id',
                    help='ID of the taken courses',  type=int)
parser.add_argument('-u', '--user_name',
                    help='Name of the user to recommend new courses')
args = vars(parser.parse_args())
# action='store_const',

#import data
df_courses = pd.read_csv('data/cleaned/df_courses.csv', index_col=0,
                         sep=' ', converters={"objectives": ast.literal_eval})
df_courses.head()
...
vectorizer_descr = TfidfVectorizer(
    stop_words=StopWords, tokenizer=udemy_functions.tokenize, max_features=1000, max_df=0.8)
...
dummies = pd.get_dummies(df_rel[['primary_subcategory', 'instructional_level', 'cluster_descr']], prefix=[
                         'subcat', 'level', 'cluster'])
df_rel.drop(columns=['primary_subcategory',
            'instructional_level', 'cluster_descr'], inplace=True)
...
if args['course_id'] != None:
    ...
    print(udemy_functions.recommend_courses(
        args['course_id'], 5, df_courses, df_norm))

if args['user_name'] != None:
    print(udemy_functions.recommend_for_user(
        args['user_name'], 5, df_reviews, df_courses, df_norm))
