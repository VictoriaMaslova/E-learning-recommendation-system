...
df = pd.read_csv('data/df_courses.csv', index_col=0)

# 2. transform the relevant columns
df['primary_category'] = udemy_functions.transform_col(
    df['primary_category'], 'title')
df['primary_subcategory'] = udemy_functions.transform_col(
    df['primary_subcategory'], 'title')
...
df['published_since_month'] = (datetime.datetime.now(
)-df['published_time']).apply(lambda x: int(x.days/30))
...
df_rating.columns = ['rating_1', 'rating_2',
...
df.drop(columns=['published_time', 'rating_distribution', 'status_label',
        'is_published', 'rating', 'description'], axis=1, inplace=True)
...
# 2. transform the relevant columns
df_review['user_name'] = udemy_functions.transform_col(
    df_review['user'], 'display_name')
df_review['user_title'] = udemy_functions.transform_col(
    df_review['user'], 'title')

# 3. filter the dataset from anonymized users (3 types)

df_review = df_review[~df_review['user_name'].isin(
    ['Anonymized User', 'Private Udemy For Business User', 'Udemy User'])]
