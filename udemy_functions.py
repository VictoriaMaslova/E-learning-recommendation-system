...
def top_words_graph(df_courses, attribute, comb_list, kind, StopWords, vocab_frame):
    for subcat in df_courses['primary_subcategory'].unique():
        ...
        if kind == 'bar':
            plt.barh(range(len(top_words)), [val[1]
                     for val in top_words], align='center')
            plt.yticks(range(len(top_words)), [val[0] for val in top_words])
            plt.xlabel("Number of occurences")
        else:
            top_words = dict(top_words)
            wordcloud = WordCloud(width=900, height=500,
                                  margin=0).generate_from_frequencies(top_words)
...
def print_common_words(common_words, word_features, vocab_frame, print_list=True):
    ...
    for num, centroid in enumerate(common_words):
        dict_cluster[num] = [
            get_label(word, vocab_frame, word_features) for word in centroid]
        if print_list:
...
def plot_common_words(model, n_words, word_features, vocab_frame, df_courses, cluster_name):
    ...
    dict_cluster = print_common_words(
        common_words, word_features, vocab_frame, False)
    ...
    colors = ['b', 'g', 'y', 'r', 'k', 'grey',
              'purple', 'orange', 'pink', 'brown']
    for j in range(len(keys)):
        ax.bar(keys[j], values[j], width=0.8, bottom=0.0, align='center',
               color=colors[j], alpha=0.4, label=dict_cluster[j])
    ax.set_xticks(np.arange(len(values)))
    ax.set_xticklabels(['cluster '+str(k) for k in keys])
    ax.set_ylabel('Number of courses')
    ax.set_title('Distribution of clusters with the top ' +
                 str(n_words) + ' words')
    plt.legend(fontsize=13)


def squarify_words(common_words, word_features, vocab_frame):
    colormaps = ['Purples', 'Blues', 'Greens', 'Oranges', 'Reds', 'Greys', 'YlOrBr', 'YlOrRd',
                 'OrRd', 'PuRd', 'RdPu', 'BuPu', 'GnBu', 'PuBu', 'YlGnBu', 'PuBuGn', 'BuGn', 'YlGn']
    for num, centroid in enumerate(common_words):
        sizes = np.arange(10, 10+len(centroid))
        cmap_name = colormaps[num]
        cmap = plt.get_cmap(cmap_name)
        labels = [get_label(word, vocab_frame, word_features)
...
def heatmap_categories_cluster(cluster_name, df_courses, cmap):
    ...
    sns.heatmap(clusters.unstack(
        level='primary_subcategory'), ax=ax, cmap=cmap)
    ax.set_xlabel('primary_subcategory', fontdict={
                  'weight': 'bold', 'size': 24})
...
def plot_dendrogram(z, last_p_show, line_dist=None):
    ...
    dendrogram(z, orientation='top', leaf_rotation=90, p=last_p_show,
...
def plot_with_pca(X, labels, plot_n_sample):
    ...
    print('The explained variance through the first 2 principal comonent is {}.'. format(
...
def normalize_features(df):
    ...
    for col in df_norm.columns:
        df_norm[col] = StandardScaler().fit_transform(
...
def recommend_for_user(user_name, n_courses, df_reviews, df_courses, df_norm):
    list_courses = df_reviews[df_reviews['user_name']
                              == user_name]['course_id'].values
    len_courses = len(list_courses)
    index_courses = df_courses[df_courses['id'].isin(list_courses)].index
    for course_id in list_courses:
        title, df_recommend = recommend_courses(
            course_id, n_courses, df_courses, df_norm)
        print('The following courses are recommended after taking the course {} with the id {}:'.format(
            title.values[0], course_id))
        print(df_recommend)
        print()
    if len_courses > 1:
        n_courses = n_courses+1
        ...
        df_temp = df_temp.sort_values(
            'avg_cos_sim', ascending=False).reset_index(drop=True)
